<p align="center">
  <img height="80" src="logo_solidworks.png">
</p>

<h1 align="center">🛠️ SolidWorks Portfolio</h1>

<p align="left">
  <img src="https://img.shields.io/badge/License-CC_BY--NC--SA_4.0-lightgrey?style=flat-square&logo=creative-commons" alt="Creative Commons">
  <img src="https://img.shields.io/badge/Software-SolidWorks-D32D2C?style=flat-square&logo=solidworks&logoColor=white" alt="SolidWorks">
  <img src="https://img.shields.io/badge/Focus-Design_for_Manufacturing_(DfM)-success?style=flat-square" alt="Design for Manufacturing">
  <img src="https://img.shields.io/github/last-commit/aliozkanme/SolidWorks-Portfolio" alt="last commit"/> 
</p>

---
## Introduction
As an experienced Electrical-Electronics Engineer, I leverage strong SolidWorks capabilities to bridge the gap between abstract technical specifications and physical mechanical design. My focus is on creating high-fidelity 3D representations and assemblies that ensure seamless mechanical integration, functional accuracy, and manufacturability across **diverse engineering disciplines**.
This repository serves as a comprehensive portfolio of my 3D CAD modeling work. It features a wide range of designs, from **precision electronic components** and **electromechanical machines** to **general mechanical assemblies**. Each model is engineered with a focus on strict adherence to datasheets, dimensional accuracy, and practical application in simulation or manufacturing.

## Projects

## ⚖️ License

This project is licensed under a **Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License**.

<p align="center">
  <a rel="license" href="http://creativecommons.org/licenses/by-nc-sa/4.0/">
    <img alt="Creative Commons License" style="border-width:0" src="https://i.creativecommons.org/l/by-nc-sa/4.0/88x31.png" />
  </a>
</p>

### 🚫 Terms of Use
* **Non-Commercial:** You may not use this material (Source Code, Designs, Models, Simulations, Documentation) for commercial purposes.
* **Attribution:** You must give appropriate credit to the author (**Ali Özkan**).
* **ShareAlike:** If you remix, transform, or build upon the material, you must distribute your contributions under the same license as the original.

<br />
To view a copy of this license, visit <a href="http://creativecommons.org/licenses/by-nc-sa/4.0/">http://creativecommons.org/licenses/by-nc-sa/4.0/</a>.

## 🤝 Credits
All credits, references, and used standard components are listed within the individual project descriptions respectively.
<p align="center">
<img src="https://img.shields.io/badge/License-CC_BY--NC--SA_4.0-lightgrey?style=flat-square&logo=creative-commons" alt="Creative Commons">
</p>